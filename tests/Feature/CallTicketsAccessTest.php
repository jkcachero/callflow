    ...
    $response->assertStatus(Response::HTTP_OK)
        ->assertInertia(fn ($page) =>
            $page->component('CallTicket/Index')
                ->where('callTickets.data', fn ($tickets) =>
                    collect($tickets)->every(fn ($ticket) => $ticket['assigned_user_id'] === $this->agent->id)
                )
        );
});

it('allows admin or supervisor to see all call tickets', function () {
    foreach ([$this->admin, $this->supervisor] as $user) {
        $response = $this->actingAs($user)->get(route('call-tickets.index'));

        $response->assertStatus(Response::HTTP_OK)
            ->assertInertia(fn ($page) =>
                $page->component('CallTicket/Index')
                    ->where('callTickets.data', fn ($tickets) =>
                        collect($tickets)->contains(fn ($ticket) => $ticket['id'] === $this->agentTickets->first()->id)
